...
def initalizeCentroids(dataset, k):
  ...
  centroids = dataset[np.random.choice(rows, size=k, replace=False), :] 
  return centroids
...
def computeAssignments(dataset, centroids):
  ...
  distList = np.ones(len(dataset)) * np.inf
  for i in range(len(centroids)):
    arr = dataset-centroids[i]
    for j in range(len(arr)):
      dist = np.linalg.norm(arr[j])
      if (dist < distList[j]):
        assignments[j] = i
        distList[j] = dist
  return assignments
...
def updateCentroids(dataset, centroids, assignments):
  sum = np.zeros(len(dataset[0]))
  counts = []
  count = 0
  for i in range(len(centroids)):
    for j in range(len(assignments)):
      if (i == assignments[j]):
        sum = np.add(sum,dataset[j])
        count += 1
    if (count != 0):
...
def calculateSSE(dataset, centroids, assignments):
  ...
  for i in range(len(dataset)):
    dist = np.linalg.norm(dataset[i]-centroids[int(assignments[i])])
    sse += pow(dist,2)
  return sse
